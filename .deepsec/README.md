...
Currently configured project: `web` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/web/SETUP.md` to fill in
   `data/web/INFO.md`.
...
`--project-id web` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/web/`.
