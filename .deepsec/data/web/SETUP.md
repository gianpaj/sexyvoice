...
This is a deepsec scanning workspace. Project `web` was just registered
(target: `..`). Setup is incomplete — `data/web/INFO.md`
...
2. **Fill in `data/web/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id web
pnpm deepsec process --project-id web
```
