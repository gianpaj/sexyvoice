# Agent setup for `sexyvoice-2`

This is a deepsec scanning workspace. Project `sexyvoice-2` was just registered
(target: `..`). Setup is incomplete — `data/sexyvoice-2/INFO.md`
...
2. **Fill in `data/sexyvoice-2/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id sexyvoice-2
pnpm deepsec process --project-id sexyvoice-2
```
