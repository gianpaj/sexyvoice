...
- `apps/web/tests/` - Vitest tests and utilities; Playwright e2e tests under
  `apps/web/e2e/` (run with `pnpm --filter @sexyvoice/web test:e2e`).
...
- `docs/devops.md` - deployment configuration and release verification.
...
pnpm lint                # Biome lint check for web and docs
pnpm format              # Biome format check for web and docs
...
- Keep `docs/devops.md` limited to deployment configuration, infrastructure,
  secrets, and release verification. Put application behavior in
  `ARCHITECTURE.md` or an existing feature README, and investigation procedures
  in `scripts/README.md`. Link to the canonical document rather than duplicating it.
