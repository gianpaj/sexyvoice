...
```json
{ "displayName": "<name>", "langCode": "EN_US",
...
```json
{ "text": "<text>", "voiceId": "<from clone>", "modelId": "inworld-tts-2",
  "deliveryMode": "CREATIVE", "language": "en-US", "audioConfig": { "speakingRate": 1 } }
```
...
- Add an `'inworld'` branch to `getDollarCost(provider, credits, text)` (currently switches
  `mistral`/`replicate` at ~line 394). Rate is **$25 per 1M characters** for the
  `inworld-tts-2` model: `(text.length / 1_000_000) * 25` (i.e. `text.length * 0.000025`).
  Define it as a named constant (e.g. `INWORLD_TTS2_DOLLARS_PER_MILLION_CHARS = 25`).
