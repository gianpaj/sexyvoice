...
### Stop immediate history-query refetches

The history page hydrates audio rows and their exact count, then client queries may
consider that data stale immediately. Introduce shared query options with a
bounded `staleTime` and preserve mutation-driven invalidation.
...
- History hydration does not immediately duplicate the same data and count reads.
...
- focused updates to `apps/web/lib/supabase/queries.client.ts` and history page
  components;
