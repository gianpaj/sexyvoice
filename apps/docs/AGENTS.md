...
- Framework: Fumadocs (Next.js + `fumadocs-ui` + `fumadocs-mdx`)
- Format: MDX files with YAML frontmatter under `content/docs`
- Config: `source.config.ts` for MDX/collection config; navigation via
  `meta.json` files
- Components: `fumadocs-ui` components
- OpenAPI reference is generated by `src/scripts/generate-docs.mts`
  (`pnpm --filter @sexyvoice/docs generate-openapi-docs`)
