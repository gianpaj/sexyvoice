...
export default async function HistoryPage(props: {
  ...
  const [
    ...
    : await Promise.all([
        getMyAudioFilesQuery(supabase, userId),
        getMyAudioFilesCountQuery(supabase, userId),
        supabase
          .from('api_keys')
  ...
  queryClient.setQueryData(['audio_files', userId], audioFiles);
  queryClient.setQueryData(
    ['audio_files_count', userId],
  ...
  return (
    <HydrationBoundary state={dehydrate(queryClient)}>
      <div className="container mx-auto pb-10">
        ...
        <DataTable showApiColumns={(apiKeysCount ?? 0) > 0} userId={userId} />
      </div>
    </HydrationBoundary>
  );
}
