...
export async function GET(request: Request) {
  ...
  if (!hmacKey) {
    return APIErrorResponse('ALTCHA_HMAC_KEY is not configured', 500);
  }
  ...
  try {
    ...
    if (!rateLimit.success) {
      return APIErrorResponse('Rate limit exceeded', 429);
    }
    ...
    return APIErrorResponse('Failed to generate Altcha challenge', 500);
  }
}
