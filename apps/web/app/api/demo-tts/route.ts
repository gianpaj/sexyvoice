...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!(text && voiceId && altchaPayload)) {
      return APIErrorResponse('missing_fields', 400);
    }
    ...
    if (!captchaValid) {
      return APIErrorResponse('invalid_captcha', 400);
    }
    ...
    if (!allowed) {
      return withSessionCookie(
        APIErrorResponse('limit_reached', 429, { remaining: 0 }),
      );
    }
    ...
    if (voiceError || !voice) {
      return withSessionCookie(APIErrorResponse('invalid_voice', 400));
    }
    ...
    if (!(demoBucket && demoPublicUrl)) {
      ...
      return withSessionCookie(APIErrorResponse('generation_failed', 500));
    }
