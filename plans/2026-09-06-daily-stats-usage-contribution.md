...
The [daily-stats README](../apps/web/app/api/daily-stats/README.md) defines the implemented report and its limitations.
...
- `apps/web/app/api/daily-stats/route.ts` renders contribution and call costs from the shared cost resolver. Retail credit value is separate from provider costs.
- `apps/web/app/api/daily-stats/contribution-queries.ts` reads cost inputs for 30 days. Transaction queries supply all-time purchase history.
...
Return an amount plus a cost basis: recorded, estimated, or unknown. Recorded means stored by the application, not invoice-verified. Track record counts by basis.
...
Use `usage_events` as the primary ledger for every feature, including ordinary calls by free users spending free credits. Use a separate paginated contribution query covering 30 days with source ID, model, dollar amount, input characters, duration, and metadata. Derive contribution windows from these results; keep existing activity reads separate. Do not filter events to paying users before calculating costs.
...
Read call sessions as supplementary data, with session ID, user ID, start/end time, duration, model, and status. Fetch sessions linked to ledger entries when needed for model/duration evidence, plus finalized sessions ending in the reporting window to find missing events. Use stable pagination and existing internal-user exclusions. For terminal legacy sessions without an end time, use `started_at` as the attribution fallback.

For each ledger call, prefer valid recorded cost after the writer audit, then a supported model-specific estimate using linked session evidence. Count its cost once. A missing linked session does not invalidate an otherwise usable event; surface missing evidence when it prevents costing.

Add a session-only cost record only when no matching `live_call` event exists. Check matching `source_id` values in batches without restricting that lookup to the report window, so an event across midnight cannot cause the session to be counted twice. If an event exists, its `occurred_at` determines inclusion; otherwise attribute the supplemental record by session end time. Do not synthesize consumed credits for missing events or infer that a free customer lacks an event.
...
Do not add session-only estimates for ongoing calls. Include terminal sessions with provider usage even if their status is not successful. Best-effort or delayed usage insertion remains a telemetry limitation; do not invent costs for completely unlogged attempts.

Contribution reads bypass the local activity cache and fetch usage and transaction history together. Query failures must fail the contribution calculation visibly, never become empty/zero-cost results. Do not invoke the live GET route during verification because it sends Telegram messages.
...
Include dashboard, API, and audio-processing costs in the paid/free totals. The report does not display per-feature free usage credits or costs.

Show unpriced and unclassified record counts next to contribution when present. Keep each period to two lines. Cost-basis subtotals, per-feature breakdowns, and supplemental/pending call counts are not report fields. Use "coverage incomplete" for any unpriced or unclassified record; alert thresholds follow the daily-stats README.
...
Update `docs/devops.md` with definitions, cost limitations, cache behavior, and safe verification instructions. Decision records live under `.agents/notes/implemented/reporting/`. No migrations, backfills, billing changes, new provider calls in the cron, or new package are required by this plan.

## Later handoff prompt for sexyvoice-admin

Implement the daily-stats contribution metrics in sexyvoice-admin after the SexyVoice implementation is complete. Read `plans/2026-09-06-daily-stats-usage-contribution.md` in the SexyVoice repository and inspect its final `usage-costs.ts`, `contribution.ts`, queries, tests, and operational documentation. Use the daily-stats README and implementation as the contract.
...
Extend the financial overview and daily series with net collections, free/paid costs, total measured costs, estimated contribution, and free-cost coverage. Show unpriced and unclassified record counts when present and match the report's incomplete-coverage labels. Per-feature credit/cost breakdowns, cost-basis subtotals, and supplemental/pending call counts are outside this handoff. Label the result before payment fees and fixed costs; do not call it accounting profit. Keep paid-user engagement metrics clearly scoped.
