...
const getPolicyByLocale = (lang: Locale) => {
  const localized = allPolicyPages.find(
    (policy) => policy.slug === POLICY_SLUG && policy.locale === lang,
  );
  ...
  return allPolicyPages.find(
    (policy) =>
      policy.slug === POLICY_SLUG && policy.locale === routing.defaultLocale,
  );
};
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `https://sexyvoice.ai/${lang}/${POLICY_SLUG}`,
      languages: Object.fromEntries(
        routing.locales.map((locale) => [locale, `/${locale}/${POLICY_SLUG}`]),
      ),
    },
  } satisfies Metadata;
}
...
export default async function PrivacyPolicyPage(props: {
  ...
  if (!routing.locales.includes(lang)) {
    redirect(`/${routing.defaultLocale}/${POLICY_SLUG}`);
  }
