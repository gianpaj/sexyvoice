...
def load_and_clean_data(file_path):
    ...
    if 'type' in df.columns:
        ...
        print(f"Filtered from {initial_count} to {
              filtered_count} transactions (purchase/topup only)")
...
def create_hourly_heatmap(df, save_path):
    ...
    heatmap_data = df.groupby(
        ['day_of_week_num', 'hour']).size().unstack(fill_value=0)
    ...
    plt.title('Transaction Heatmap: Hour of Day vs Day of Week',
...
def create_monthly_trends(df, save_path):
    ...
    axes[0, 0].plot(x_pos, monthly_transactions.values,
    ...
    if 'user_id' in df.columns:
        ...
        axes[0, 1].plot(x_pos, monthly_users.values, marker='s',
    ...
    if 'dollarAmount' in df.columns:
        ...
        axes[1, 0].plot(x_pos, monthly_revenue.values, marker='^',
        ...
        axes[1, 1].plot(x_pos, monthly_avg.values, marker='d',
...
def create_user_behavior_charts(df, save_path):
    ...
    axes[0, 0].hist(user_transaction_counts, bins=min(
        20, user_transaction_counts.max()), alpha=0.7, color='skyblue')
    ...
    axes[0, 1].pie(segments, labels=labels, colors=colors,
    ...
    if 'dollarAmount' in df.columns:
        ...
        axes[1, 0].hist(user_revenue, bins=min(
            20, len(user_revenue.unique())), alpha=0.7, color='lightgreen')
        ...
        axes[1, 1].hist(df['dollarAmount'], bins=min(
            30, len(df['dollarAmount'].unique())), alpha=0.7, color='orange')
...
def create_daily_patterns(df, save_path):
    ...
    axes[0].bar(range(len(daily_counts)), daily_counts.values,
    ...
    axes[1].plot(hourly_counts.index, hourly_counts.values,
...
def create_transaction_types_analysis(df, save_path):
    ...
    if 'month' in df.columns:
        monthly_types = df.groupby(
            ['month', 'type']).size().unstack(fill_value=0)
...
def create_transaction_amounts_analysis(df, save_path):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    fig.suptitle('Transaction Dollar Amounts Analysis',
                 fontsize=16, fontweight='bold')
    ...
    labels = ['$5', '$10', '$99']
    df_amounts['amount_range'] = pd.cut(
        df_amounts['dollarAmount'], bins=bins, labels=labels, include_lowest=True)

    amount_counts = df_amounts['amount_range'].value_counts()
    axes[0].pie(amount_counts.values, labels=amount_counts.index,
                autopct='%1.1f%%', startangle=90)
    axes[0].set_title('Dollar Amount Distribution')

    # Dollar amounts over time
    if 'month' in df_amounts.columns:
        monthly_amounts = df_amounts.groupby(
            'month')['dollarAmount'].agg(['sum', 'mean', 'count'])

        revenue_color = "darkgreen"
        average_color = "orange"
        # Create a bar chart for total revenue per month
        x_pos = range(len(monthly_amounts))
        axes[1].bar(x_pos, monthly_amounts['sum'], alpha=0.7,
                    color=revenue_color, label='Total Revenue')

        # Add a line for average transaction amount
        ax2 = axes[1].twinx()
        ax2.plot(x_pos, monthly_amounts['mean'], color=average_color,
                 marker='o', linewidth=2, label='Avg Amount')
        ...
        axes[1].set_xticklabels([str(m)
                                for m in monthly_amounts.index], rotation=45)
...
def get_weekly_comparison_data(df):
    ...
    if len(current_month_data) > 0:
        ...
        current_month_data['week_of_month'] = get_week_of_month(
            current_month_data['created_at'], current_month_start)
    ...
    if len(prev_month_data) > 0:
        ...
        prev_month_data['week_of_month'] = get_week_of_month(
            prev_month_data['created_at'], prev_month_start)
    ...
    if 'dollarAmount' in df.columns:
        ...
        current_revenue_data = current_month_data[current_month_data['dollarAmount'].notna(
        )] if len(current_month_data) > 0 else pd.DataFrame()
        if len(current_revenue_data) > 0:
            current_weekly_revenue = current_revenue_data.groupby('week_of_month')[
                'dollarAmount'].sum()
            current_revenue = [current_weekly_revenue.get(
                week, 0) for week in weeks]
        else:
            current_revenue = [0] * len(weeks)

        # Filter revenue data for previous month
        prev_revenue_data = prev_month_data[prev_month_data['dollarAmount'].notna()] if len(
            prev_month_data) > 0 else pd.DataFrame()
        if len(prev_revenue_data) > 0:
            prev_weekly_revenue = prev_revenue_data.groupby('week_of_month')[
                'dollarAmount'].sum()
...
def create_weekly_mtd_comparison(df, save_path):
    ...
    weeks, current_counts, prev_counts, (current_month_start,
                                         prev_month_start), current_revenue, prev_revenue = weekly_data
    ...
    fig.suptitle('Weekly Comparison: MTD vs Previous Month',
                 fontsize=16, fontweight='bold')
    ...
    ax1 = axes[0]
    bars1 = ax1.bar(x - width/2, current_counts, width,
                    label='MTD', color='steelblue', alpha=0.8)
    bars2 = ax1.bar(x + width/2, prev_counts, width,
                    label='Prev Month', color='darkorange', alpha=0.8)
    ...
    if has_revenue:
        ax2 = axes[1]
        bars3 = ax2.bar(x - width/2, current_revenue, width,
                        label='MTD', color='green', alpha=0.8)
        bars4 = ax2.bar(x + width/2, prev_revenue, width,
                        label='Prev Month', color='red', alpha=0.8)
        ...
        revenue_summary = f"MTD Revenue: ${
            current_revenue_total:.2f}\nPrev Month: ${prev_revenue_total:.2f}"
        if prev_revenue_total > 0:
            revenue_change_pct = (
                (current_revenue_total - prev_revenue_total) / prev_revenue_total) * 100
...
def create_dashboard(df, save_path):
    ...
    fig.suptitle('SexyVoice.ai Credit Transactions Dashboard',
                 fontsize=20, fontweight='bold')

    # Key metrics (top row)
    ax1 = fig.add_subplot(gs[0, 0])
    ax1.text(0.5, 0.7, f"{len(df):,}", ha='center',
             va='center', fontsize=24, fontweight='bold')
    ax1.text(0.5, 0.3, "Total Transactions",
    ...
    if 'user_id' in df.columns:
        ...
        ax2.text(0.5, 0.7, f"{unique_users:,}", ha='center',
                 va='center', fontsize=24, fontweight='bold')
        ax2.text(0.5, 0.3, "Unique Users",
                 ha='center', va='center', fontsize=12)
    ax2.set_xlim(0, 1)
    ax2.set_ylim(0, 1)
    ax2.axis('off')
    ...
    if 'dollarAmount' in df.columns:
        ...
        ax3.text(0.5, 0.7, f"${total_revenue:,.0f}", ha='center',
                 va='center', fontsize=24, fontweight='bold')
    ...
    if 'dollarAmount' in df.columns:
        ...
        ax4.text(0.5, 0.7, f"${avg_transaction:.2f}", ha='center',
                 va='center', fontsize=24, fontweight='bold')
        ax4.text(0.5, 0.3, "Avg Transaction",
                 ha='center', va='center', fontsize=12)
    ax4.set_xlim(0, 1)
    ax4.set_ylim(0, 1)
    ax4.axis('off')
    ...
    if 'day_of_week' in df.columns:
        day_order = ['Monday', 'Tuesday', 'Wednesday',
                     'Thursday', 'Friday', 'Saturday', 'Sunday']
        daily_counts = df.groupby('day_of_week').size().reindex(day_order)
        ax5.bar(range(len(daily_counts)), daily_counts.values,
                color='steelblue', alpha=0.8)
    ...
    if weekly_data[0] is not None:
        ...
        bars1 = ax7.bar(x - width/2, current_counts, width,
                        label='MTD', color='steelblue', alpha=0.8)
        bars2 = ax7.bar(x + width/2, prev_counts, width,
                        label='Prev Month', color='darkorange', alpha=0.8)
    ...
    if weekly_data[0] is not None and current_revenue is not None and prev_revenue is not None:
        bars3 = ax7_revenue.bar(
            x - width/2, current_revenue, width, label='MTD', color='green', alpha=0.8)
        bars4 = ax7_revenue.bar(x + width/2, prev_revenue,
                                width, label='Prev Month', color='red', alpha=0.8)
    ...
    if 'month' in df.columns:
        monthly_transactions = df.groupby('month').size()
        ax8.plot(range(len(monthly_transactions)),
                 monthly_transactions.values, marker='o', linewidth=2)
...
def main():
    ...
    create_transaction_amounts_analysis(
        df, f"{base_name}_transaction_amounts.png")
