...
**How we do it differently:** One platform and one credit balance across pre-generated speech, cloned voices, and live conversation — instead of assembling a TTS vendor, a cloning vendor, and a realtime voice stack. (One *platform*, not one API: the API itself is speech-generation only.)
