...
/**
 * Checks if the current selection is a valid text selection (non-empty)
 */
...
/**
 * Gets a display name for the currently selected node
 */
export const getNodeDisplayName = (editor: Editor): string => {
  const { selection } = editor.state
...
/**
 * Checks if the selection is valid (checking for empty selection and other conditions)
 */
export const isSelectionValid = (editor: Editor | null): boolean => {
...
export const getSelectionBoundingRect = (editor: Editor): DOMRect | null => {
  ...
  // Try to use the native selection range first which is often accurate for text
