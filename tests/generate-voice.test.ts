...
describe('Generate Voice API Route', () => {
  ...
  describe('Credit System', () => {
    it('should return 402 when user has insufficient credits', async () => {
      ...
      const request = new Request('http://localhost/api/generate-voice', {
        ...
        body: JSON.stringify({ text: 'Hello world this is a test', voice: 'tara' })
      })
  ...
  describe('Voice Generation - Google Gemini', () => {
    ...
    it('should fallback to flash model when pro model fails', async () => {
      ...
      let callCount = 0
      server.use(
        http.post('https://generativelanguage.googleapis.com/v1beta/models/*:generateContent', ({ request }) => {
          callCount++
          if (callCount === 1) {
            // First call (pro model) fails
            return HttpResponse.json({ error: 'Pro model failed' }, { status: 500 })
          } else {
